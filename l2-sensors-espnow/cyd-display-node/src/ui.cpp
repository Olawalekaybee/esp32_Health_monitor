...
void ui_create(void) {
    ...
    lv_obj_set_style_pad_all(status_banner, 10, 0);
    ...
    status_label = lv_label_create(status_banner);
    lv_label_set_text(status_label, "Waiting for main-node...");
    lv_obj_set_style_text_font(status_label, FONT_VALUE, 0);
    lv_obj_set_style_text_color(status_label, COLOR_TEXT_PRIMARY, 0);
    lv_obj_align(status_label, LV_ALIGN_TOP_LEFT, 0, 0);

    reason_label = lv_label_create(status_banner);
    lv_label_set_text(reason_label, "");
    lv_obj_set_style_text_font(reason_label, FONT_LABEL, 0);
    lv_obj_set_style_text_color(reason_label, COLOR_TEXT_MUTED, 0);
    lv_obj_set_width(reason_label, 280);
    // Scrolling ticker instead of wrapping: long reasons (e.g. "Both
    // sensors down (DHT22 x20, DS18B20 x20)") were getting cut off
    // before; this scrolls them continuously instead.
    lv_label_set_long_mode(reason_label, LV_LABEL_LONG_SCROLL_CIRCULAR);
    lv_obj_align(reason_label, LV_ALIGN_BOTTOM_LEFT, 0, 0);
