...
#define STATUS_STRIP_H  20   // Layer 6: WiFi/SD/Cloud mini-badges row
#define CARD_GAP        8
#define CARD_Y          (TITLE_BAR_H + ACCENT_LINE_H + STATUS_STRIP_H + 6)
#define CARD_H          112  // unchanged from Layer 2 — the status strip's
                              // height comes out of the banner below instead,
                              // so the already-verified sparkline/label
                              // positions inside each card don't shift.
#define CARD_W          148
#define BANNER_Y        (CARD_Y + CARD_H + CARD_GAP)
#define BANNER_H        (240 - BANNER_Y - 8)

// --- System-status strip: 3 equal-width mini-badges spanning the
//     same margins as the sensor cards below them ---
#define STATUS_BADGE_GAP 8
#define STATUS_BADGE_W   ((320 - 2 * CARD_GAP - 2 * STATUS_BADGE_GAP) / 3)
#define STATUS_STRIP_Y   (TITLE_BAR_H + ACCENT_LINE_H + 4)
