...
static void onReceive(int bytes) {
    if (bytes != sizeof(SensorHealthPacket)) {
        // Most common cause: main-node and cyd-display-node are running
        // firmware built against different versions of comms_packet.h
        // (e.g. one side got the wire struct extended and reflashed,
        // the other didn't). Previously this dropped silently forever,
        // which looks identical to "no connection at all" — logging it
        // turns an invisible mismatch into an obvious one.
        Serial.printf("[i2c] size mismatch: got %d bytes, expected %d — "
                      "main-node and cyd-display-node likely have different "
                      "comms_packet.h versions. Rebuild+reflash both.\n",
                      bytes, (int)sizeof(SensorHealthPacket));
