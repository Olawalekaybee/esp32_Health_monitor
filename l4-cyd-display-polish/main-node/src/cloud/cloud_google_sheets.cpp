                     ...
                     const HealthReport &health)
{
    ...
    // Apps Script Web Apps genuinely respond with a 302 on a normal
    // successful doPost — Google's infra redirects to serve the actual
    // response body from googleusercontent.com, but the script (and
    // therefore the sheet write) has already run by that point. A
    // negative code means HTTPClient never got a response at all
    // (DNS/TLS/connection failure); 4xx/5xx means Google's servers
