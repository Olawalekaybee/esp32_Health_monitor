...
void storageTask(void *pvParameters) {
    ...
    // Publish the mount result for comms_task to relay to the CYD —
    // this only changes once (at boot), so a single read-modify-write
    // here is enough; no ongoing race with other tasks' fields since
    // this task never touches statusQueue again after this point.
    SystemStatus status = {};
    xQueuePeek(statusQueue, &status, 0);
    status.sd_card_ok = sdReady;
